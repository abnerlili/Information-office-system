...
if($link)
{
    ...
    else
    {
        ...
        //展示队旗
        $sql="select * from teamflag where active='1'";	//选取活跃值（active）为1的数据进行展示
        $result=mysql_query($sql,$link);
        if($result)
        {
            echo "<center>
            <table width=\"320\" border=\"1\">
            <caption>队旗交接</caption>
            <tr>
            ...
            while($row=mysql_fetch_assoc($result))
            {
                ...
                echo "<tr>";
                echo "<td align='center'>"."{$row['flagid']}</td>";
                echo "<td align='center'>{$row['changetime']}</td>";
                echo "<td align='center'>$bumen</td>";
                echo "<td align='center'>";
                ?>
        
        <form action="delaction.php" onsubmit="return confirm('你确定队旗现在在你这吗？');" style="margin:0px">
            <input type="hidden" name="action" value="getflag" />
            <input type="hidden" name="flagid" value="<?php echo "{$row['flagid']}";?>" />
        ...
        <form action="delaction.php" onsubmit="return confirm('你确定你把这个队旗放活动室了吗？');" style="margin:0px">
            <input type="hidden" name="action" value="returnflag" />
            <input type="hidden" name="flagid" value="<?php echo "{$row['flagid']}";?>" />
