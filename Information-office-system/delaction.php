...
switch($_GET["action"]){
    ...
    break;
     ...
     //队章归属
    case "getseal":
    $sealid=$_GET['sealid'];					//获取交接队章编号
    $updateseal1="update team_seal set active='0' where sealid='$sealid'";//更改已交接队章活跃值为0
    mysql_query($updateseal1,$link);
    $sql9="insert into team_seal values('$sealid',NULL,'$_SESSION[ID]','1',NULL)";//插入新的数据
    ...
    case "return_seal":
    $sealid=$_GET['sealid'];					//获取交接队章编号
    $updateseal2="update team_seal set active='0'  where sealid='$sealid'";//更改已交接队章活跃值为0
    mysql_query($updateseal2,$link);
    $sql10="insert into team_seal values('$sealid',NULL,'200','1',NULL)";//插入新的数据
    mysql_query($sql10,$link);
    //自动跳转回浏览页面
    header("Location:teamseal.php");
    break;
    //更多待定
